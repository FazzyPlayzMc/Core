...
public class FspeedCmd implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0){
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("fspeed-format")));
            ...
            if (p.hasPermission("core.command.fspeed")){
                ...
                if (a <= 0){
                    ...
                    try {
                        p.setFlySpeed(a);
                        p.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("set-fspeed")) + a);
